...
import { okabeIto, palettes, type ThemeName } from './colors.ts';
import { encodingsFor } from './encodings.ts';
...
// Both themes pass the same checks.
describe.each(['light', 'dark'] as ThemeName[])('%s theme', (theme) => {
  const colors = palettes[theme];
  const { dotStyles, replicaStyles, seriesStyles } = encodingsFor(colors);
