...
| Engine modules | `src/engine/core/` (E2), `src/engine/shared/`, topics in `src/engine/core/ids.ts` | How engine modules plug together: `defineModule` and `createDayRunner` (README in `core/`); the shared request table (slots, field ownership, slot reuse) and meters (who writes which counters and levels); the request-lifecycle topics requestArrived → requestDispatched → firstToken → requestEnded, plus requestCancelled, requestState, and replicaState. Module order for E11: shared, load (E6), router (E7), replica (E5), failure (E8), metrics (E9). |
...
| 5 | Sessions moved by the crash: ≥ 0.8 under mod-N, ≤ 0.2 under consistent hashing. Over the 3 minutes after the crash vs. the 15 before (recovery takes ~2 min, so the damage is two short spikes, at mark-down and at rejoin): mod-N fleet prefill ≥ 1.8×, TTFT mean ≥ 1.8×, p99 ≥ 1.3×, returning hit rate drops ≥ 0.15; consistent hashing's TTFT rise ≤ half of mod-N's. Under least-outstanding, the crashed replica draws ≥ 80% of requests before mark-down (K34) and the rejoined one ≥ 60% of the first 2 s, with history hits ≤ 0.05 in its first 30 s. (Refined in C3b.) |
| 6 | Immediate retries with 10 s timeouts, crash at the peak: amplification ≥ 2× and goodput < 50% of a no-crash run for the first 2 minutes (the storm lasts about as long as the outage, ~2–3 min, because abandoned sessions shed load), then a lasting ~10% goodput shortfall. With full-jitter backoff and admission control: admitted ÷ first attempts ≤ 1.1 (offered load still ~2×, since rejected requests retry), goodput ≥ 0.9 over minutes 5–15, and at most half the abandoned sessions. (Refined in C3c.) |
...
**G1 decisions (September 2026, from S1).** Chunks are 5 simulated minutes, with a 1-minute chunk first after init or a fork and one chunk for each off-shift night. Checkpoints every 15 simulated minutes, kept for the playhead's day. bucketMs 10 s and histBucketMs 60 s, emitted over each day's active window only. Storage per K28; histograms per K29. E4's allocate-evict-register-release cycle was to drop from about 140 ns to 30 ns or less (follow-up WP E4b, same API). E4b reached 39–42 ns (a whole Server B Wednesday went from ~11.7 s to ~9.3 s; checkpoints 3.6 → 3.0 MB) and was accepted: P1 passes at a 09:30 entry, and the rest is bookkeeping the API requires.
