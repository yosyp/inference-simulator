...
| K33 | Chart details (decided in U4) | — | Chart 1 uses a log y axis: E2E p99 sits 10–100× above TTFT and would flatten the TTFT lines that lead (K14). Charts never show time after the playhead. The default window is the playhead's shift day, with zoom steps of 4 h, 2 h, 1 h, 30 min, and 15 min that page to keep the playhead in view. Sparse data: p99 points widen until a typical point holds 50 requests; below 12 points the chart plots individual requests. In High-side mode, chart 3 shows daily bars only for utilization; other chart 3 kinds become "not collected" panels (§9). |
| K35 | Dark mode and keyboard shortcuts (author request, after v1) | — | A dark palette for every token, toggled top right; it follows prefers-color-scheme until the visitor picks, and the choice is in memory only (no browser storage). Both themes pass the same contrast and colour-blindness checks. `?` opens a shortcuts modal: Space play/pause, r reset, j jump to lesson, [ ] prev/next tab, 1–6 tabs, h High side, − = speed, p parameters, t trigger, d theme, Esc close. Shortcuts are ignored while typing and while another modal is open. Each tab also carries a lesson box (summary and takeaway) under the sidebar title, and opens at 50×. |
